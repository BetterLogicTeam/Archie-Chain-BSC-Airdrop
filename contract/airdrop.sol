...
contract airdrop is Ownable{
   ...
   function multisendToken(IBEP20 _token, address[] memory _contributors, uint256[] memory _balances) public onlyOwner   
        {   
            require(_contributors.length==_balances.length,"the arrays length are not same!");
            IBEP20 tokentoSend=IBEP20(_token);
    ...
    function sendMultiBNB(address payable[]  memory  _contributors, uint256[] memory _balances) public  payable  
    {
        ...
        emit Multisended(  msg.value , msg.sender);
          ...
          function multisendTokenwithApprove(IBEP20 _token, address[] memory _contributors, uint256[] memory _balances) public   
        {   
            require(_contributors.length==_balances.length,"the arrays length are not same!");
            IBEP20 tokentoSend=IBEP20(_token);
           uint256  totalVal=AirdropAmount(_balances);
        ...
        function AirdropAmount(uint256[] memory _amounts) public  view returns(uint256){
           ...
           function multisendTokenSigle(IBEP20 _token, address[] memory _contributors, uint256 _balances) public onlyOwner  
        {   
  ...
  function emergencyWithdraw(IBEP20 _token,uint256 _tokenAmount) public onlyOwner {
         IBEP20(_token).transfer(msg.sender, _tokenAmount);
    }
